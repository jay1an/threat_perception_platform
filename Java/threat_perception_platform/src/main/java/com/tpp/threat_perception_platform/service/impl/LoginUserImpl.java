...
public class LoginUserImpl implements LoginServcie {
    ...
    public ResponseResult<Object> login(User user) {
        ...
        String jwt = JwtUtil.createJWT(uuid,JwtUtil.JWT_TTL);  // JWT令牌中也要设置一下过期时间
    ...
    public ResponseResult<?> changePassword(ChangePasswordRequest request) {
        ...
        // 认证当前密码是否正确
        try {
            authenticationManager.authenticate(new UsernamePasswordAuthenticationToken(currentUsername, request.getCurrentPassword()));
        } catch (Exception e) {
            return new ResponseResult<>(1001, "当前密码错误");
        }
        // 更新密码
